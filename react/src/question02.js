...
class Question02 extends Component {
  ...
  increaseTimer(){
    ...
    if(this.state.seconds_timer >= this.state.seconds_wait){
      this.props.nextPage([1, this.props.qIndex, -1, this.state.timestamp])
      this.setState({ timestamp: Date.now() , seconds_timer: 0});
    } else {
      this.timeouts.push(setTimeout(this.increaseTimer.bind(this), 1000))
    }
        ...
        <div className="row justify-content-md-center">
          <div className="col-4 text-center">
            <button type="button" className="btn btn-primary" style={{ width: '200px' }} onClick={() => {
              ...
              this.setState({ timestamp: Date.now() , seconds_timer: 0});
              this.props.nextPage([1, this.props.qIndex, this.props.data[this.props.qIndex].imgA, old_timestamp])
            }} >Option A</button>
            <img onClick={() => {
              let old_timestamp = this.state.timestamp
              this.setState({ timestamp: Date.now() , seconds_timer: 0});
          ...
          <div className="col-4">
            ...
            <img style={{ border: '2px solid #A79AFF' }} className="img-fluid" alt="scatterplot" src={this.props.data[this.props.qIndex].imgR.scatter}/>
          </div>
          <div className="col-4 text-center">
            <button type="button" className="btn btn-primary" style={{ width: '200px' }} onClick={() => {
              let old_timestamp = this.state.timestamp
              this.setState({ timestamp: Date.now() , seconds_timer: 0});
              this.props.nextPage([1, this.props.qIndex, this.props.data[this.props.qIndex].imgB, old_timestamp])
            }} >Option B</button>
            <img onClick={() => {
              let old_timestamp = this.state.timestamp
              this.setState({ timestamp: Date.now() , seconds_timer: 0});
